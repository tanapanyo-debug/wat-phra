...
eq(html.indexOf('"กรรมการฝ่าย" + dept') >= 0, true, "public-works committee sits on monk history");
eq(html.indexOf('"ผู้ช่วยเลขานุการฝ่าย" + dept') >= 0, true, "public-works deputy secretary sits on monk history");
eq(html.indexOf('"ประธานฝ่าย" + dept') >= 0 && html.indexOf("ศึกษาสงเคราะห์") >= 0, true, "other sangha departments have the same offices");
eq(html.indexOf('"เลขานุการฝ่าย" + dept') >= 0 && html.indexOf('"เผยแผ่"') >= 0, true, "propagation department has a secretary office");
...
eq(html.indexOf('source: "wat-phra"') >= 0, true, "embed tells accounting who is in the frame");
eq(html.indexOf('pingParent("duty-daily-saved")') >= 0, true, "saving chanting notifies accounting history");
eq(html.indexOf('pingParent("duty-opened")') >= 0, true, "opening chanting notifies accounting history");
...
eq(serverSrc.indexOf("/api/duties/roster") >= 0, true, "duty roster api");
...
eq(serverSrc.indexOf("/api/scholarships") >= 0, true, "scholarship api");
eq(html.indexOf('id="nav-fund"') >= 0, true, "scholarship tab");
eq(isDutiesAllowedPath("/scholarships"), false, "scholarships not duties-only");
...
eq(html.indexOf('id="btn-rains-xlsx"') >= 0, true, "rains excel download button");
eq(html.indexOf("/api/report.xlsx") >= 0, true, "rains excel download url");
